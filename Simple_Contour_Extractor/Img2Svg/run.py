...
Executables are taken from <root>/bin (then PATH) and every file is written to <root>/output.
    ...
    Img2Svg/
    |-- bin/      csv2json  img2csv  json2svg
    |-- images/   logo.png  sample.png          (inputs for --input)
    |-- output/   <name>.csv .json .svg _overlay.png   (+ <name>.png when generated)
    `-- run_pipeline.py

Usage:
    python3 run_pipeline.py --input images/logo.png [--epsilon N] [--name NAME] [--no-overlay]
    python3 run_pipeline.py [--width N] [--height N] [--noise N] [--seed N] [--epsilon N]
...
PALETTE = [(255, 64, 64), (64, 220, 64), (80, 140, 255), (255, 200, 40), (220, 80, 220), (40, 220, 220)]
SVG_NS = "{http://www.w3.org/2000/svg}"
NUM = r"\d+(?:\.\d+)?"
PATH_RE = re.compile(rf"M {NUM} {NUM}(?: L {NUM} {NUM})*(?: Z)?")  # the only path syntax json2svg emits
...
def read_gray(path):
    ...
    rows = [bytes(map(int, line.split(","))) for line in path.read_text().splitlines() if line.strip()]
...
def render_scene(width, height, noise, seed):
    ...
    tri = [(0.55 * width, 0.88 * height), (0.90 * width, 0.88 * height), (0.725 * width, 0.55 * height)]
    sides = list(zip(tri, tri[1:] + tri[:1]))

    def in_triangle(x, y):
        # Inside when the point is on the same side of all three edges.
        return len({(bx - ax) * (y - ay) - (by - ay) * (x - ax) >= 0 for (ax, ay), (bx, by) in sides}) == 1
...
def validate_json(doc, width, height):
    ...
    if (doc.get("width"), doc.get("height")) != (width, height):
        raise ValueError(f"size mismatch: JSON says {doc.get('width')}x{doc.get('height')}, image is {width}x{height}")
    for expected_id, edge in enumerate(doc["edges"], start=1):
        if edge["id"] != expected_id:
            raise ValueError(f"contour ids must be sequential: got {edge['id']}, expected {expected_id}")
        points = edge["points"]
        if not points:
            raise ValueError(f"contour {expected_id} has no points")
        for p in points:
            if not (0 <= p["x"] < width and 0 <= p["y"] < height):
                raise ValueError(f"contour {expected_id}: point {p} is outside the image")
        for a, b in zip(points, points[1:]):
            if max(abs(a["x"] - b["x"]), abs(a["y"] - b["y"])) != 1:
                raise ValueError(f"contour {expected_id}: points {a} -> {b} are not 8-neighbours")
...
def validate_svg(path, width, height, contour_ids):
    ...
    if (root.get("width"), root.get("height"), root.get("viewBox")) != (str(width), str(height), f"0 0 {width} {height}"):
        raise ValueError(f"size mismatch: {root.get('width')}x{root.get('height')}, viewBox '{root.get('viewBox')}'")
    paths = root.findall(f"{SVG_NS}path")
    total = 0
    for element in paths:
        d = element.get("d", "")
        if not PATH_RE.fullmatch(d):
            raise ValueError(f"unexpected path data (only M, L and Z are allowed): '{d[:60]}'")
        if element.get("data-id") not in {str(i) for i in contour_ids}:
            raise ValueError(f"path refers to an unknown contour id: {element.get('data-id')}")
        coords = [(float(x), float(y)) for x, y in COORD_RE.findall(d)]
        if not all(0 <= x <= width and 0 <= y <= height for x, y in coords):
            raise ValueError(f"contour {element.get('data-id')}: a point is outside the viewBox")
        total += len(coords)
...
def find_exe(name, override):
    ...
    candidates = [override, f"./{override}"] if override else [str(ROOT / "bin" / name), name]
    for candidate in candidates:
        path = shutil.which(candidate)
        if path is not None:
            return str(Path(path).absolute())  # steps run with cwd=ROOT, so never pass a cwd-relative path
    where = f"'{override}'" if override else f"{ROOT / 'bin'} or in PATH"
    sys.exit(f"Error: executable '{name}' not found: looked for {where} (build the project or use --{name})")


def run_step(title, cmd):
    print(f"{title}\n    $ {shlex.join([shown(cmd[0]), *cmd[1:]])}")
    proc = subprocess.run(cmd, capture_output=True, text=True, cwd=ROOT)  # same relative paths as printed
    for line in proc.stdout.splitlines():
        print(f"    {line}")
    if proc.returncode != 0:
        sys.stderr.write(proc.stderr)
        print(f"Error: '{Path(cmd[0]).name}' failed with exit code {proc.returncode}", file=sys.stderr)
...
def main():
    ...
    parser.add_argument("--input", help="PNG to process (default: generate a test image)")
    parser.add_argument("--name", help="base name of the output files (default: the input file name, or 'example')")
    parser.add_argument("--out-dir", default=str(ROOT / "output"), help=f"where the files are written (default: {ROOT / 'output'})")
    parser.add_argument("--epsilon", type=float, default=1.0, help="Ramer-Douglas-Peucker tolerance (default: 1.0)")
    parser.add_argument("--no-overlay", action="store_true", help="skip the contour overlay PNG (about +2 s per 6 megapixels)")
    parser.add_argument("--img2csv", help="img2csv executable (default: <root>/bin/img2csv, then PATH)")
    parser.add_argument("--csv2json", help="csv2json executable (default: <root>/bin/csv2json, then PATH)")
    parser.add_argument("--json2svg", help="json2svg executable (default: <root>/bin/json2svg, then PATH)")
    generated = parser.add_argument_group("generated test image (not allowed together with --input)")
    generated.add_argument("--width", type=int, help="default: 256")
    generated.add_argument("--height", type=int, help="default: 192")
    generated.add_argument("--noise", type=int, help="+/- random gray noise per pixel (default: 3)")
    ...
    if args.input and given:
        parser.error(f"--{given[0]} only applies to the generated test image (not allowed with --input)")
    ...
    csv, js, svg, overlay = (out / f"{name}{suffix}" for suffix in (".csv", ".json", ".svg", "_overlay.png"))
    ...
    if args.input:
        ...
        write_png(png, width, height, render_scene(width, height, args.noise, args.seed), 1)

    run_step("[2/5] img2csv (PNG -> CSV)", [img2csv, "-i", shown(png), "-o", shown(csv)])
    run_step("[3/5] csv2json (CSV -> contours)", [csv2json, "-i", shown(csv), "-o", shown(js)])
    run_step("[4/5] json2svg (contours -> SVG)", [json2svg, "-i", shown(js), "-o", shown(svg), "-e", str(args.epsilon)])
    ...
    try:
        paths, svg_points = validate_svg(svg, width, height, [e["id"] for e in doc["edges"]])
    ...
    if not args.no_overlay:
        try:
            ...
            if (gray_width, gray_height) != (width, height):
                raise ValueError(f"CSV is {gray_width}x{gray_height}, image is {width}x{height}")
        except ValueError as error:
            sys.exit(f"Error: invalid {shown(csv)}: {error}")
        write_png(overlay, width, height, render_overlay(gray, width, height, doc["edges"]), 3)

    json_points = sum(len(e["points"]) for e in doc["edges"])
    print(f"    contours: {len(doc['edges'])}, points: {json_points} (JSON) -> {svg_points} (SVG, {paths} paths)")
    produced = ([] if args.input else [png]) + [csv, js, svg] + ([] if args.no_overlay else [overlay])
